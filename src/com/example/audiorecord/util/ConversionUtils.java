...
/**
 * 转换工具类，将字段转换为hashtable，将hashtable转换为json字符串等 将文件转换为字节数组等
 * 
 * @author zhangzhilai
 * 
 */
public class ConversionUtils {
	 ...
	 * @return
	 */
	public static Hashtable<String, Serializable> generateLanguage(long leagueId, int currentPage, int pageSize, int userId, int topicAttrType) {
		Hashtable<String, Serializable> htable = new Hashtable<String, Serializable>();
		htable.put("LeagueID", leagueId);
		htable.put("CurrentPage", currentPage);
		htable.put("PageSize", pageSize);
		htable.put("UserID", userId);
		htable.put("TopicAttrType", topicAttrType);
		return htable;
	}
	...
	public static void writeEnd(String fileName, byte[] content) {
		try {
			...
			String contentString = Base64.encodeToString(content, Base64.DEFAULT);
